...
We will use the Titanic dataset provided by University of Washington, Information School for the course of Data Science I. The Titanic dataset contains data about the survival of passengers aboard the Titanic. 
...
```{r}
library(Matrix)
#converts train set factors to columns
sparse.matrix.train <- sparse.model.matrix(survived~pclass + sex + title -1, 
                                           data = train) 
#converts test set factors to columns
sparse.matrix.test <-  sparse.model.matrix(survived~pclass + sex + title -1, 
                                           data = test.data) 
#output vector to be predicted
output_vector = train$survived 
...
xgb.model.one <- xgb.cv(data= sparse.matrix.train,     #train sparse matrix
                       label= as.numeric(output_vector)-1, #output vector to be predicted 
                       eval.metric = 'logloss', #model minimizes RootMeanSquaredError
                       ...
                       nround = 100  #Vary btwn 100-3000 
                       #based on max.depth, eta,subsample & colsample
                       )
plot(data.frame(xgb.model.one)[,1], type='l', col='black', 
     ylab='CV logloss Error', xlab='# of trees')
lines(data.frame(xgb.model.one)[,3], type='l', lty=2, col='black')
```

The above function performs Cross Validation using XGBoost package. Using this function we can tune the parameters of model to minimize the evaluation matrix which here is the Mean Logloss error or Classification Loss error. The function is given the training data in form of sparse matrix. The objective parameter specifies the model to be fitted. The original data is partitioned into nfold number of samples and the cross validation process is repeated for nround of times to generate the most optimized values. By tuning the parameters we can tune the model.

The plot provides the Cross Validation Root Mean Square Error rate vs the number of trees created by model. The solid line gives the Training Logloss Mean error while the dotted line gives the Test Logloss Mean error. The plot tells us that as the number trees increases the logless error both taining as well as test decreases until it becomes almost constant after about 100 trees.This tells us that as we increase the number of trees in the model the accuracy of model will increase.

Now we will modify the code to fit a boosted tree model that allows for 8 levels in each tree and uses a learning rate $\eta=.1$. Then we will compare the two models.
```{r, results="hide"}

#new model with depth = 8 and eta as 0.1
...
str(data.frame(xgb.model.two))
#plotting error curves for the two models to compare
plot(data.frame(xgb.model.two)[,1], type='l', col='black', 
     ylab='CV logloss Error', xlab='# of trees', 
     main = "Logloss errors vs Trees for two xgBoost Models")
lines(data.frame(xgb.model.two)[,3], type='l', lty=2, col='black')
lines(data.frame(xgb.model.one)[,1], type='l',  col='blue')
lines(data.frame(xgb.model.one)[,3], type='l', lty=2, col='blue')
legend("topright", lty=c(1,1), lwd=c(2.5,2.5),col=c("blue", "black" ),
       legend= c('xgBoost1', 'xgBoost2'))
```

The Logless error curves for the two models can be compared using the plot. For the second model we can see that the training as well as the test mean errors falls to 0.45  with only about 20 trees while the first model takes 40 trees (almost double) to reduce the errors to that level. So, obviously the second model is using better optimized parameters and hence we will choose it over the first one.

### References ###
Data and some content taken from University of Washington, Information School.
